@php
    $siteDetails = \App\Models\Settings::first();
...
<head>
    <meta charset="utf-8" />
    <title>@yield('title', 'Easy Mart') </title>
...
<body>
    <!-- Quick view -->
    @include('frontend.homeSections.viewModel')
    <!-- Header  -->

    @include('frontend.homeSections.header')
   ...
   <!-- End Header  -->
            ...
            <div class="container pt-15 pb-20">
                <div class="row">
                        ...
                        <div class="widget-about font-md mb-md-3 mb-lg-3 mb-xl-0 wow animate__animated animate__fadeInUp" data-wow-delay="0">
                            ...
                            <ul class="contact-infor">
                                <li><img src="{{ asset('frontend/assets/imgs/theme/icons/icon-location.svg') }}" alt="" /><strong>Address: </strong> 
                                    <span>{{ $siteDetails->address }}</span></li>
                                <li><img src="{{ asset('frontend/assets/imgs/theme/icons/icon-contact.svg') }}" alt="" /><strong>Call Us:</strong>
                                    <span>{{ $siteDetails->phone }}</span></li>
                                <li><img src="{{ asset('frontend/assets/imgs/theme/icons/icon-email-2.svg') }}" alt="" /><strong>Email:</strong>
                                    <span>{{ $siteDetails->email }}</span></li>
                    ...
                    <div class="footer-link-widget col wow animate__animated animate__fadeInUp" data-wow-delay=".3s">
                        ...
                        <ul class="footer-list mb-sm-5 mb-md-0">
                            <li><a href="{{ route('becomeVendor') }}">Become a Vendor</a></li>
            ...
            <div class="row align-items-center">
                ...
                <div class="col-xl-4 col-lg-6 col-md-6 text-end d-none d-md-block">
                    <div class="mobile-social-icon">
                        <h6>Follow Us</h6>
                        <a href="{{ $siteDetails->facebook }}"><img src="{{ asset('frontend/assets/imgs/theme/icons/icon-facebook-white.svg') }}" alt="" /></a>
                        <a href="{{ $siteDetails->twitter }}"><img src="{{ asset('frontend/assets/imgs/theme/icons/icon-twitter-white.svg') }}" alt="" /></a>
                        <a href="{{ $siteDetails->instagram }}"><img src="{{ asset('frontend/assets/imgs/theme/icons/icon-instagram-white.svg') }}" alt="" /></a>
                        {{-- <a href="#"><img src="{{ asset('frontend/assets/imgs/theme/icons/icon-pinterest-white.svg') }}" alt="" /></a> --}}
                        <a href="{{ $siteDetails->youtube }}"><img src="{{ asset('frontend/assets/imgs/theme/icons/icon-youtube-white.svg') }}" alt="" /></a>
                    </div>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function () {
            let wishlistItems = document.querySelectorAll('.wishlist-product');
            wishlistItems.forEach(item => {
                item.addEventListener('click', (event) =>{
                    event.preventDefault();
                    // Get the product ID from the data attribute
                    let productId = item.getAttribute('data-product-id');
                    fetch('/product/add/wishlist/' + productId, {
                        'method' : 'GET'
        ...
        // Compare Product add
        document.addEventListener('DOMContentLoaded', function () {
            let compareItems = document.querySelectorAll('.compare-product');
            compareItems.forEach(item => {
                item.addEventListener('click', (event) =>{
                    event.preventDefault();
                    // Get the product ID from the data attribute
                    let productId = item.getAttribute('data-product-id');
                    fetch('/product/add/compare/' + productId, {
                        'method' : 'GET'
                    }).then( (response) => response.json())
                    .then( (result) => {
                        miniCartData();
