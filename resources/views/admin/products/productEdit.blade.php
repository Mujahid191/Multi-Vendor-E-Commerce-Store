...
<div class="page-content">
    ...
    <div class="card">
      <div class="card-body p-4">
        <form method="POST" action="{{ route('adminProductUpdate') }}" enctype='multipart/form-data' class="needs-validation" novalidate>
          @csrf
          @method('put')
          <div class="row">
            <div class="col-lg-8">
            ...
            <div class="col-lg-4">
              <div class="border border-3 p-4 rounded">
                <div class="row g-3">
                  ...
                  <input type="text" class="form-control" name="discount" id="inputCompareatprice" value="{{ $product->discount_price }}">
                  </div>
                  <div class="col-md-12">
                    ...
                    <div class="mb-3 select2-sm">
                      <select name="vendor" class="single-select">
                        @foreach ($vendors as $item)
                          <option {{ $product->user_id == $item->id ? 'selected' : '' }} value="{{ $item->id}}">{{ $item->name}}</option>
                        @endforeach
                      </select>
                    </div>
                  </div>
    ...
    <div class="card">
      <div class="card-body">
        <form method="POST" action="{{ route('adminProductUpdateMultiImage') }}" enctype='multipart/form-data'>
          @csrf
          @method('put')
              ...
              @foreach ( $multiImages as $key => $image )
                <tr>
                  ...
                  <td>
                    ...
                    <a href="{{ route('adminProductDeleteMultiImage', $image->id ) }}" class="btn btn-danger btn-sm ms-3" id="delete">Delete</a>
                  </td>
                </tr>
              @endforeach
            </tbody>
