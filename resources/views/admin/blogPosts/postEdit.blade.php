...
<div class="page-content">
    ...
    <div class="card">
        <div class="card-body">
          <form action="{{ route('adminBlogPostUpdate') }}" method="POST" enctype='multipart/form-data' class="needs-validation" novalidate>
            @csrf
            @method('put')
            <div class="row mb-3">
              <div class="col-sm-3">
                ...
                <select name="blog_category" class="single-select" required>
                  <option value="" disabled selected>Select Category</option>
                    @foreach ($blogCategories as $category)
                      <option {{ $post->BlogCategory_id == $category->id ? 'selected' : '' }} value="{{ $category->id }}">{{ $category->category_name }}</option>
                    @endforeach
                </select>                
            ...
            <div class="row mb-3">
              ...
              <div class="col-sm-9 text-secondary">
                <input type="text" name="short_description" value="{{ $post->short_description }}" class="form-control" required/>
            ...
            <div class="row mb-3">
              ...
              <div class="col-sm-9 text-secondary">
                <textarea id="mytextarea" name="long_description"  placeholder="Long Description" required>{{ $post->long_description }}</textarea>
            ...
            <div class="row mb-3">
              ...
              <div class="col-sm-9 text-secondary">
                <img id="preview" class="border" src="{{ $post->image == '' ? asset('backend/assets/images/no_image.jpg') : asset('images/blogs/' . $post->image) }}" alt="post Picture" width="100">
              </div>
            </div>
