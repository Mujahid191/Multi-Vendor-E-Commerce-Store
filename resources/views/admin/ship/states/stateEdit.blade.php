...
<div class="page-content">
    ...
    <div class="page-breadcrumb d-none d-sm-flex align-items-center mb-3">
        ...
        <div class="ms-auto">
            <a href="{{ route('adminStatesAll') }}" class="btn btn-primary">Back</a>
        </div>
    </div>
    <!--end breadcrumb-->
    <div class="card">
        <div class="card-body">
          <form action="{{ route('adminStateUpdate') }}" method="POST" enctype='multipart/form-data' class="needs-validation" novalidate>
            @csrf
            @method('put')
              ...
              <div class="col-sm-9 text-secondary">
                <select name="division" class="single-select" onchange="loadDistrict()" id="division" required>
                  ...
                  @foreach ($divisions as $item)
                    <option {{ $item->id == $state->division_id ? 'selected' : '' }} value="{{ $item->id}}">{{ $item->name}}</option>
                  @endforeach
                </select>
